...
# Queryset containing all game objects in the DB
...
class Game(models.Model):
    ...
    def board(self):
        ...
        enabling you  to ask for the state of the field at position [y][x]"""
    ...
    def _get_game_status_after_move(self, move):
        x, y = move.x, move.y
        board = self.board()
        if (board[y][0] == board[y][1] == board[y][2] is not None) or \
                (board[0][x] == board[1][x] == board[2][x] is not None) or \
...
class Move(models.Model):
    ...
    #     """Method doesn't seem to be called, TODO: fix this"""
    #     if other is None:
    #         return False
